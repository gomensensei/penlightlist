...
function renderGrid(targetCanvas = canvas, targetCtx = ctx, isDownload = false) {
  ...
  if (document.getElementById("showKonmei").checked) {
    ...
    const konmei = customKonmei || (konmeiSelect === 'other' ? '' : konmeiSelect);
  ...
  for (let i = 0; i < grid.length; i++) {
    ...
    if (document.getElementById("showPhoto").checked && member.name_ja in preloadedImages) {
      ...
      let drawHeight = cellH - 20;
      let drawWidth = drawHeight * aspectRatio;
      if (drawWidth > halfW) {
        drawWidth = halfW;
        drawHeight = drawWidth / aspectRatio;
      }
      targetCtx.drawImage(img, x + 5, yOffset, drawWidth, drawHeight);
    }
    ...
    yOffset = y + 10;
    if (yOffset + 40 <= y + cellH) {
      targetCtx.fillStyle = '#F676A6';
      targetCtx.font = '28px KozGoPr6N'; // 增大字型
      targetCtx.fillText(member.name_ja, x + halfW + 5, yOffset);
      yOffset += 35;
    }
    if (document.getElementById("showNickname").checked && yOffset + 28 <= y + cellH) {
      targetCtx.font = '24px KozGoPr6N';
      targetCtx.fillText(member.nickname, x + halfW + 5, yOffset);
      yOffset += 30;
    }
    if (document.getElementById("showKi").checked && yOffset + 28 <= y + cellH) {
      targetCtx.font = '24px KozGoPr6N';
      targetCtx.fillText(member.ki, x + halfW + 5, yOffset);
      yOffset += 30;
    }
    if (document.getElementById("showColorBlock").checked || document.getElementById("showColorText").checked) {
      ...
      if (document.getElementById("showColorBlock").checked && yOffset + 80 <= y + cellH) {
        member.colors.forEach((color, j) => {
          targetCtx.fillStyle = color === '#FFFFFF' ? '#f0f0f0' : color;
          targetCtx.fillRect(x + halfW + 5 + j * 80, yOffset, 80, 80); // 增大色塊
        });
        yOffset += 85;
...
function setupEventListeners() {
  ...
  document.getElementById('customKonmei').addEventListener('input', () => {
    renderGrid(); // 即時更新，清除殘影
    ctx.clearRect(0, 0, canvas.width, 30); // 清除標題區域殘影
  });
  ...
  document.getElementById('popupSelectBtn').addEventListener('click', () => {
    console.log('Select button clicked, currentIndex:', currentIndex);
  ...
  document.getElementById('downloadButton').addEventListener('click', async () => {
    try {
      ...
      renderGrid(tempCanvas, tempCtx, true);
